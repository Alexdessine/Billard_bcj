...
class AdminNationalLink extends AdminController
{
     ...
     * @var string
     */
    protected $title = 'Lien CueScore national';
    ...
    {
        $grid = new Grid(new NationalLink());
        ...
        $grid->column('master', __('Blackball Master'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
        $grid->column('mixte_tableau_a', __('Mixte tableau a'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
        $grid->column('mixte_tableau_b', __('Mixte tableau b'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
        $grid->column('feminin', __('Feminin'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
        $grid->column('espoir', __('Espoir'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
        $grid->column('junior', __('Junior'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
        ...
        $grid->column('individuel', __('Individuel'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
        $grid->column('equipe', __('Equipes'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
        $grid->column('doublette', __('Doublettes'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : "-");
...
document.getElementById("voir-inscrits")?.addEventListener("click", function () {
    ...
    const colonnes = ["master", "mixte_tableau_a", "mixte_tableau_b", "feminin", "espoir", "junior", "benjamin", "handi", "veteran", "individuel", "equipe", "doublette"];
...
JS);
    ...
    {
        $show = new Show(NationalLink::findOrFail($id));

        // $show->field('id', __('Id'));
        $show->field('calendrier.titre', __('Tournoi'));
        $show->field('master', __('Master'));
        $show->field('mixte_tableau_a', __('Mixte tableau a'));
        $show->field('mixte_tableau_b', __('Mixte tableau b'));
        $show->field('feminin', __('Feminin'));
        $show->field('espoir', __('Espoir'));
        $show->field('junior', __('Junior'));
        $show->field('handi', __('Handi'));
        $show->field('veteran', __('Veteran'));
        $show->field('individuel', __('Individuel'));
        $show->field('equipe', __('Equipes'));
        $show->field('doublette', __('Doublettes'));
    ...
    {
        $form = new Form(new NationalLink());
        ...
        if ($form->isCreating()) {
            ...
            $calendrier = Calendrier_national::find(request('calendrier_id'));
            if ($calendrier) {
        ...
        $form->url('master', __('Master'));
        $form->url('mixte_tableau_a', __('Mixte tableau a'));
        $form->url('mixte_tableau_b', __('Mixte tableau b'));
        $form->url('feminin', __('Feminin'));
        $form->url('espoir', __('Espoir'));
        $form->url('junior', __('Junior'));
        $form->url('handi', __('Handi'));
        $form->url('veteran', __('Veteran'));
        // Ajout d’un encadré avec indication
        $form->html('<div style="border:1px solid #ccc; padding: 15px; border-radius: 5px; margin-top: 20px; margin-bottom:20px; background-color: #f9f9f9;">
        <strong>⚠️ Ces champs sont à renseigner uniquement pour la coupe de France</strong><br>');

        $form->url('individuel', __('Individuel'));
        $form->url('equipe', __('Equipes'));
        $form->url('doublette', __('Doublettes'));
        $form->html('</div>');
