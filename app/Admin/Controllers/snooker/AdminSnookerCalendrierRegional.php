...
class AdminSnookerCalendrierRegional extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Calendrier régional - Discipline snooker';
    ...
    {
        $grid = new Grid(new SnookerCalendrierRegional());
        ...
        $grid->column('actions', __('Liens'))->display(function () {
            $linkExists = \App\Models\SnookerRegionalLink::where('calendrier_id', $this->id)->exists();

            if ($linkExists) {
                return '<a href="/admin/snooker/liens_cuescore/regional-links?calendrier_id=' . $this->id . '" class="btn btn-sm btn-primary">Voir les liens Cuescore</a>';
            } else {
                return '<a href="/admin/snooker/liens_cuescore/regional-links/create?calendrier_id=' . $this->id . '" class="btn btn-sm btn-warning">Ajouter les liens Cuescore</a>';
            }
        });
    ...
    {
        $show = new Show(SnookerCalendrierRegional::findOrFail($id));
    ...
    {
        $form = new Form(new SnookerCalendrierRegional());
