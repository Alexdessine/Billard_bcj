...
class AdminSnookerCalendrierDepartemental extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Calendrier départemental - Discipline snooker';
    ...
    {
        $grid = new Grid(new SnookerCalendrierDepartemental());
        ...
        $grid->column('actions', __('Liens'))->display(function () {
            $linkExists = \App\Models\SnookerDepartementalLink::where('calendrier_id', $this->id)->exists();

            if ($linkExists) {
                return '<a href="/admin/snooker/liens_cuescore/departemental-links?calendrier_id=' . $this->id . '" class="btn btn-sm btn-primary">Voir les liens Cuescore</a>';
            } else {
                return '<a href="/admin/snooker/liens_cuescore/departemental-links/create?calendrier_id=' . $this->id . '" class="btn btn-sm btn-warning">Ajouter les liens Cuescore</a>';
            }
        });
    ...
    {
        $show = new Show(SnookerCalendrierDepartemental::findOrFail($id));
    ...
    {
        $form = new Form(new SnookerCalendrierDepartemental());
