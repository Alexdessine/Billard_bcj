...
class AdminCalendrierRegional extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Calendrier régional';

    public function index(Content $content)
    {
        return $content
            ->title($this->title)
            ->description('Liste des liens vers le calendrier régional')
            ->body($this->grid());
    }
    ...
    {
        $grid = new Grid(new Calendrier_regional());
        ...
        // $grid->column('date_limite', __('Date limite d\'inscription'));
        ...
        $grid->column('actions', __('Liens'))->display(function () {
            $linkExists = RegionalLink::where('calendrier_id', $this->id)->exists();

            if ($linkExists) {
                return '<a href="/admin/regional-links?calendrier_id=' . $this->id . '" class="btn btn-sm btn-primary">Voir les liens Cuescore</a>';
            } else {
                return '<a href="/admin/regional-links/create?calendrier_id=' . $this->id . '" class="btn btn-sm btn-warning">Ajouter les liens Cuescore</a>';
            }
        });
    ...
    {
        $show = new Show(Calendrier_regional::findOrFail($id));
        ...
        $show->field('date_limite', __('Date limite d\'inscription'))->display(function ($value) {
            ...
            if ($now->greaterThan($dateLimit)) {
                return '<p class="text-danger"><s>' . e($dateLimit->format('Y-m-d')) . '</s></p>';
            } else {
        ...
        $show->field('url', __('Lien SportEasy'));
    ...
    {
        $form = new Form(new Calendrier_regional());
        ...
        $form->url('url', __('Lien SportEasy'));
