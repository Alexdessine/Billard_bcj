...
class AdminAmericainCalendrierNational extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Calendrier national - Discipline américain';
    ...
    {
        $grid = new Grid(new AmericainCalendrierNational());
        ...
        $grid->column('actions', __('Liens'))->display(function () {
            $linkExists = \App\Models\AmericainNationalLink::where('calendrier_id', $this->id)->exists();

            if ($linkExists) {
                return '<a href="/admin/americain/liens_cuescore/national-links?calendrier_id=' . $this->id . '" class="btn btn-sm btn-primary">Voir les liens Cuescore</a>';
            } else {
                return '<a href="/admin/americain/liens_cuescore/national-links/create?calendrier_id=' . $this->id . '" class="btn btn-sm btn-warning">Ajouter les liens Cuescore</a>';
            }
        });
    ...
    {
        $show = new Show(AmericainCalendrierNational::findOrFail($id));
    ...
    {
        $form = new Form(new AmericainCalendrierNational());
