...
use Encore\Admin\Widgets\Modal;
use App\Models\Calendrier_regional;
use OpenAdmin\Admin\Controllers\AdminController;

class AdminRegionalLink extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Liens CueScore régional';
    ...
    {
        $grid = new Grid(new RegionalLink());

        // Récupérer le paramètre GET s'il existe
        ...
        $grid->column('top_ligue', __('Top ligue'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : '-');
        ...
        $grid->column('handi_fauteuil', __('Handi Fauteuil'))->display(fn($v) => $v ? "<a href='{$v}' target='_blank' style='text-decoration:none; color:#000;'>{$v}</a>" : '-');
...
document.getElementById("voir-inscrits")?.addEventListener("click", function () {
    ...
    const colonnes = ["top_ligue", "mixte", "feminin", "U15", "U18", "U23", "handi", "handi_fauteuil", "veteran"];
...
JS);
    ...
    {
        $show = new Show(RegionalLink::findOrFail($id));
    ...
    {
        $form = new Form(new RegionalLink());
        ...
        if ($form->isCreating()) {
            ...
            $calendrier = Calendrier_regional::find(request('calendrier_id'));
            if($calendrier) {
